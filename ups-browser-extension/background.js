...
function contentRead(tabId, code, mode = 'full') {
  return withTimeout(new Promise((resolve) => {
    chrome.tabs.sendMessage(tabId, { type: 'HAHOA_UPS_CONTENT', action: 'read', code, mode }, (result) => {
...
async function injectedRead(tabId, code, mode = 'full') {
  const results = await withTimeout(chrome.scripting.executeScript({
    target: { tabId }, injectImmediately: true, func: globalThis.readUpsDocument, args: [null, code, true, mode],
...
async function readTab(tabId, code, allowInstall = false, mode = 'full') {
  let content = await contentRead(tabId, code, mode);
  ...
  if (allowInstall && isMissingReceiver(bridgeError)) {
    ...
    if (installed.error) {
      ...
      content = await contentRead(tabId, code, mode);
  ...
  try {
    return { result: await injectedRead(tabId, code, mode), method: 'inject', bridgeError };
  } catch (error) {
...
async function track(code, options = {}) {
  let tabId;
  let ownsTab = false;
  let keepTab = false;
  let lastReadError = '';
  ...
  try {
    const openTabs = await withTimeout(options.fresh ? Promise.resolve([]) : queryUpsTabs(), 7000, 'QUERY_TABS_TIMEOUT');
    ...
    if (!existing) {
      for (const item of openTabs) {
        ...
        const read = await readTab(item.id, code, !installAttempted.has(item.id), options.mode);
    ...
    while (Date.now() < deadline) {
      ...
      const read = initialResult ? { result: initialResult, method: 'content' }
        : await readTab(tabId, code, !installAttempted.has(tabId), options.mode);
    ...
    if (ownsTab && Number.isInteger(tabId) && (!keepTab || options.automatic)) await chrome.tabs.remove(tabId).catch(() => {});
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  ...
  const key = `${sender.tab.id}:${code}`;
  if (!inFlight.has(key)) {
    inFlight.set(key, track(code, { mode: message.mode === 'quick' ? 'quick' : 'full',
      fresh: message.fresh === true, automatic: message.automatic === true }).finally(() => inFlight.delete(key)));
  }
  void inFlight.get(key).then(respond);
  return true;
});
